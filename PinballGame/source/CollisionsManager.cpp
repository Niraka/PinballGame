...
/**
Returns a list of Collidable objects that could potentially be colliding with the given AABB (Communicated via a point
...
@return A list containing all static objects that have potentially collided with this Collidable. */
std::list<Collidable*> CollisionsManager::getPotentialCollidables(Vector2D vPos, float fHalfWidth, float fHalfHeight)
{
	...
	// Create a list containing one copy of each collidable
	...
	for (std::vector<CollidableEdgeNode>::iterator it = m_staticCollidablesX.begin(); it != m_staticCollidablesX.end(); ++it)
	{
		if (it->isStart())
		{
			collidables.push_back(it->getCollidable());
		}
	}
	...
	for (std::vector<CollidableEdgeNode>::iterator it = m_staticCollidablesX.begin(); it != m_staticCollidablesX.end(); ++it)
	{
		if (it->isStart())
		{
			if (it->getXPos() > xEnd)
			{
				collidables.remove(it->getCollidable());
			}
		}
		else
		{
...
void CollisionsManager::checkCollisions()
{
	...
	std::list<Collidable*> potentialCollidables;
	...
	for (std::vector<Collidable*>::iterator it = m_movingCollidables.begin(); it != m_movingCollidables.end(); ++it)
	{
		...
		for (std::vector<Collidable*>::iterator itMovables = m_movingCollidables.begin(); itMovables != m_movingCollidables.end(); ++itMovables)
		{
			if (itMovables != it)
			{
				...
				{
					potentialCollidables.push_back(*itMovables);
				}
			}
		}
		...
		std::string type1;
		std::string type2;
		for (std::list<Collidable*>::iterator it2 = potentialCollidables.begin(); it2 != potentialCollidables.end(); ++it2)
		{
