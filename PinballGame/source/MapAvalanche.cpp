...
/**
Constructs a default MapAvalanche. */
MapAvalanche::MapAvalanche()
{
}

/**
Updates the map with avalanche specific logic. */
void MapAvalanche::update()
{
...
void MapAvalanche::resetMap()
{
	...
	std::vector<GameObject*> gameObjects = getGameObjects("sporgle");
...
/**
Builds the Avalanche map. The map data is loaded in from a fixed file location ("maps/avalanche.txt"). Note that
the map demands the existence of the following objects: "flipperLeft", "flipperRight". */
void MapAvalanche::build()
{
	MapBuilder mb;
	mb.buildMap("../data/maps/avalanche.txt", m_gameObjects, m_materialsManager);;
...
void MapAvalanche::collisionOccurred(std::string sName1, std::string sName2)
{
	...
	else if (sName1.find("sporgle") != std::string::npos && sName2.find("flipper") != std::string::npos)
	{
		m_scoreTracker.modScore(500.f);
		updateScoreDisplay();
	}	
	else if (sName2.find("sporgle") != std::string::npos && sName1.find("flipper") != std::string::npos)
	{
		m_scoreTracker.modScore(500.f);
		updateScoreDisplay();
	}
